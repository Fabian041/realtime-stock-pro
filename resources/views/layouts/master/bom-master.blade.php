            ...
            <div class="card" style="padding: 2rem;">
                <div class="row">
                    ...
                    <div class="col-md-2 text-end pe-1">
                        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#basicModal"><i
                                class="bx bx-import me-sm-2"></i>
                            <span class="d-none d-sm-inline-block">Import</span>
                        </button>
                    </div>
                    <div class="col-md-2 text-end">
                        <button class="btn btn-label-primary" data-bs-toggle="modal" data-bs-target="#addPart"><i
                ...
                <div class="modal-body">
                    ...
                    <form method="POST" action="{{ route('bom.master.insertData') }}" id="editUserForm" class="row g-3">
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <select class="form-select" id="id_part" aria-label="Default select example" name="id_part">
                                ...
                                @foreach ($parts as $item)
                                    <option value="{{ $item->id }}">{{ $item->part_name }} (BN:
                                        {{ $item->back_number }})</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-3">
                            ...
                            <input type="number" step="any" id="qty_use" name="qty_use"
                                class="form-control @error('qty_use') is-invalid @enderror" placeholder="1920"
                                min="1" required />
                        ...
                        <div class="col-3">
                            ...
                            <select class="form-select" id="uom" aria-label="Default select example"
                                name="uom">
            ...
            <div class="modal-dialog modal-lg modal-simple modal-edit-user">
                <div class="modal-content p-2 p-md-5">
                    <div class="modal-body">
                        ...
                        <form method="POST" action="/master/bom-master/update/{{ $bom->id }}" id="editUserForm"
                            class="row g-3">
                            @method('POST')
                            @csrf
                            <div class="col-12 col-md-6">
                                <label class="form-label" for="id_part">Part Name</label>
                                <select class="form-select" id="id_part" aria-label="Default select example"
                                    name="id_part">
                                    <option value="null" selected>Pilih Part</option>
                                    @foreach ($parts as $item)
                                        <option value="{{ $item->id }}"
                                            {{ $item->id == $bom->id ? 'selected' : '' }}>
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <select class="form-control" id="id_material" aria-label="Default select example"
                                    ...
                                    @foreach ($materials as $item)
                                        <option value="{{ $item->id }}"
                                            {{ $item->id == $bom->id_material ? 'selected' : '' }}>
                                            {{ $item->part_name }} (BN: {{ $item->back_number }})
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-6">
                                <label class="form-label" for="id_area">Area</label>
                                <select class="form-select" id="id_area" aria-label="Default select example"
                                    name="id_area">
                                    <option value="null" selected>Pilih Area</option>
                                    @foreach ($areas as $item)
                                        <option value="{{ $item->id }}"
                                            {{ $item->id == $bom->id_area ? 'selected' : '' }}>
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-3">
                                ...
                                <input type="number" step="any" id="qty_use" name="qty_use"
                                    class="form-control @error('qty_use') is-invalid @enderror" placeholder="1920"
                                    min="1" value="{{ $bom->qty_use }}" required />
